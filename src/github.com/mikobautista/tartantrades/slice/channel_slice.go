...
func (ts *Tslice) Size() int {
    x := ts.Get().([]interface{})
    return len(x)
}

func (ts *Tslice) Contains(v interface{}) bool {
    r := make(chan bool)
...
func (ts *Tslice) Append(v interface{}) {
    r := make(chan interface{})
...
func (ts *Tslice) Rem(v interface{}) {
    r := make(chan interface{})
...
func (ts *Tslice) GetStringList() []string {
    r := make(chan []string)
